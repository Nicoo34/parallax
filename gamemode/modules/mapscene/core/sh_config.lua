...
ax.localization:Register("tr", {
    ...
    ["subcategory.mapscene"] = "Scènes",

    ["config.map.scene.fov"] = "FOV",
    ["config.map.scene.fov.help"] = "Champ visuel utilisé pour les scènes de carte.",
    ["config.map.scene.smooth"] = "Lissage",
    ["config.map.scene.smooth.help"] = "Facteur de lissage pour l'interpolation de la caméra.",
    ["config.map.scene.time"] = "Temps de Transition",
    ["config.map.scene.time.help"] = "Temps en secondes pour les transitions de scènes.",
    ["config.map.scene.snap"] = "Passer instantanément entre les scènes",
    ["config.map.scene.snap.help"] = "Passer instantanément entre les scènes au lieu de les lisser.",
    ["config.map.scene.linear"] = "Mouvement Linéaire",
    ["config.map.scene.linear.help"] = "Utiliser le mouvement linéaire pour les scènes de carte au lieu de l'interpolation.",
    ["config.map.scene.input"] = "Entrée de la Souris",
    ["config.map.scene.input.help"] = "Activer le décalage de caméra basé sur la souris pendant les scènes de carte.",
    ["config.map.scene.music.path"] = "Chemin de la Musique de la Scène de Carte",
    ["config.map.scene.music.path.help"] = "Chemin du son à jouer pendant les scènes de carte (laisser vide pour désactiver).",
    ["config.map.scene.music.volume"] = "Volume de la Musique de la Scène de Carte",
    ["config.map.scene.music.volume.help"] = "Volume pour la musique de la scènes de carte (0-1).",
    ["config.map.scene.music.loopDelay"] = "Délai de Boucle de la Musique de la Scène de Carte",
    ["config.map.scene.music.loopDelay.help"] = "Secondes à attendre avant de redémarrer la piste après sa fin.",
    ["config.map.scene.music.fadeIn"] = "Fondu d'Entrée de la Musique de la Scène de Carte",
    ["config.map.scene.music.fadeIn.help"] = "Secondes pour fondu en entrée lorsque la musique démarre (0 pour instantané).",
    ["config.map.scene.music.fadeOut"] = "Fondu de Sortie de la Musique de la Scène de Carte",
    ["config.map.scene.music.fadeOut.help"] = "Secondes pour fondu en sortie lorsque la musique s'arrête (0 pour instantané).",
    ["config.map.scene.strength"] = "Force de l'Entrée",
    ["config.map.scene.strength.help"] = "Force du décalage de caméra basé sur la souris.",
    ["config.map.scene.roll"] = "Roulis de l'Entrée",
    ["config.map.scene.roll.help"] = "Roulis maximal appliqué à partir du décalage de la souris (0 pour désactiver).",
    ["config.map.scene.randomize"] = "Randomiser la Sélection de Scène",
    ["config.map.scene.randomize.help"] = "Randomiser la sélection de scènes lorsque celle-ci n'est pas ordonnée.",
    ["config.map.scene.order"] = "Mode d'Ordre des Scènes",
    ["config.map.scene.order.help"] = "Ordre de sélection des scènes pour les cartes.",
    ["config.map.scene.transition"] = "Mode de Transition",
    ["config.map.scene.transition.help"] = "Mode d'interpolation de transition.",
    ["config.map.scene.max"] = "Nombre Maximum de Scènes Enregistrées",
    ["config.map.scene.max.help"] = "Nombre maximum de scènes de carte qui peuvent être enregistrées sur le serveur.",
    ["config.map.scene.scope"] = "Portée de Persistance des Scènes",
    ["config.map.scene.scope.help"] = "Portée de persistance pour les scènes de carte enregistrées.",
    ["config.map.scene.tags.allowed"] = "Étiquettes de Scène Autorisées",
    ["config.map.scene.tags.allowed.help"] = "Liste d'autorisation optionnelle pour les étiquettes de scènes."
})
